...
template <typename T> void analysisChoiceManager(MainMenu& mainMenu, DataManager<T>& data) {
    // show menu
    ...
    for (;;) {
        ...
        // print data
            ...
            // print reference staff database
            ScreenMsg::print(data.staffDatabaseShow());
            ScreenMsg::print("Type staff name or <all> to extract all staff data >> "); 
            ...
            ScreenMsg::print("Type project name or <all> to extract all project data >> "); 
...
std::string getDataType() {
    ...
    while (dataType != "DOUBLE" && dataType != "INT" && dataType != "COMPLEXDOUBLE") {
        ...
        ScreenMsg::print("Choose data type: <double>, <int>, <complexdouble> >> ");
...
std::string getDataPath(const std::string& userOrSimFile, std::string dataType) {
    if (userOrSimFile == "F") {
        ScreenMsg::print("Enter file(s) directory (e.g. <sim_double>) >> ");
...
void dataInputChoiceManager(MainMenu& mainMenu) {
    ...
    if (choice == "SIM" || choice == "F") {
        ...
        if (dataType == "DOUBLE") {
            ...
            if (dataExists) {
                // data exists, call analysis menu
                analysisChoiceManager<int>(mainMenu, data); 
            }        
            ...
            if (dataExists) {
                // data exists, call analysis menu
                analysisChoiceManager<std::complex<double>>(mainMenu, data); 
            }
        }   
...
int main() {
	...
	* -----------------------------------------------------------------------*/
	...
	// initialise main and help menus
    ...
    while (!quit) {
        ...
        dataInputChoiceManager(mainMenu); 
    }
    return EXIT_SUCCESS;
}